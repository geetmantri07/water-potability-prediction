...
class DataValidation:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            data_df = DataValidation.read_data(file_path=self.data_ingestion_artifact.ingested_file_path)

            # Checking col len of dataframe for train/test df
            status = self.validate_number_of_columns(dataframe=data_df)
            if not status:
            ...
            status = self.is_column_exist(df=data_df)
            if not status:
                validation_error_msg += f"Columns are missing in training dataframe. "
            else:
